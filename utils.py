...
def create_load_add_head(load, model_state_dict=None, data_parallel=False, **kwargs):
...
def cal_loss_target_preserve_all_tasks(tmp_model, lamb, loss_fn, 
                                       all_x_dst=None, all_y_dst=None, x_clean=None, y_clean=None, reverse=False):
...
def generate_save_name(save_dict):
    ...
    for k in save_dict.keys():
        if type(save_dict[k]) == float or type(save_dict[k]) == int or type(save_dict[k]) == bool:
...
def save_exp_config(save_dict, file_path):
    config_to_save = {
        k: v for k, v in save_dict.items()
        if k not in {'model', 'acc_mat', 'avg_acc', 'bwt', 'dataset', 'pretrained_ckpt', 'latest_noise', 'noise', 'rnd_idx_train'}
    }
    if 'cont_method_args' in config_to_save:
        config_to_save['cont_method_args'] = {
            k: v for k, v in config_to_save['cont_method_args'].items() if k != 'model_emp'
        }
    ...
    config_to_save = convert(config_to_save)

    with open(file_path, 'w') as f:
        yaml.dump(config_to_save, f, default_flow_style=False)

def cal_loss_target_alltasks(tmp_model, loss_fn, all_x_dst=None, all_y_dst=None, reverse=False):
