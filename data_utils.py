...
def generate_split_cifar100_tasks(task_num, seed=0, rnd_order=True, order=None ):
    ...
    for i in range(task_num):
        ...
        y_train_task = torch.tensor(y_train_task) # TO FIX WARNING: y_train_task = y_train_task.clone().detach()
        y_tst_task = torch.tensor(y_tst_task) # TO FIX WARNING: y_tst_task = y_tst_task.clone().detach()
...
def get_dataset_specs(**kwargs):
    ...
    if kwargs['dataset'] == 'split_cifar100':  
        ...
        tiny_root_add = os.path.join('./', 'data', 'tiny-imagenet-200')
