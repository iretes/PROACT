...
def fisher_matrix_diag(t,x,y,model,criterion,sbatch=20, drop_last_batch=False):
    # Init
    fisher={}
    ...
    for i in tqdm(range(0,x.size(0),sbatch),desc='Fisher diagonal',ncols=100,ascii=True):
        end_idx = min(i+sbatch,x.size(0))
        if drop_last_batch and end_idx - i < sbatch:
            continue
        b=torch.LongTensor(np.arange(i,end_idx))
        images=x[b].to(device)  
        target=y[b].to(device)  
